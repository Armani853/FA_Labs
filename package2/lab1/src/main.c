...
void run_result(const double epsilon) {
    printf("Запуск набора тестов");
    {
        const int base = 10;
        const double ex_fractions[] = {1.0 / 4.0, 1.0 / 5.0, 1.0 / 7.0, 1.0 / 2.0, 4.0 / 5.0};
        const int count = 5;
        bool results[count];

        StatusCode s = check_finite(base, epsilon, &results[0], count, ex_fractions[0], ex_fractions[1], ex_fractions[2], ex_fractions[3], ex_fractions[4]);
        printf("\nТест 1: Основание: 10\n");
        print_status(s);
        const bool expected[] = {true, true, true, true, true};
        bool flag = true;
...

    {
        ...
        const int count = 4;
        bool results[count];
        StatusCode s = check_finite(base, epsilon, results, count, ex_fractions[0], ex_fractions[1], ex_fractions[2], ex_fractions[3]);
...
int main() {
    ...
    run_result(EPS);
    StatusCode s = check_finite(base, EPS, results, num_fractions, frac1, frac2, frac3, frac4);
    if (s == STATUS_OK) {
